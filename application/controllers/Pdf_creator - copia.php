...
public function lista_pagos(){
      if(isset($this->session->userdata['logged_in']) && $this->session->userdata['logged_in']['privilegios']==99){
            $this->form_validation->set_rules('id_periodo', 'Periodo', 'trim|required|xss_clean|numeric');
            if( $this->form_validation->run() == FALSE ){
                  echo "No se eligió periodo";
            }else{
                  $this->load->model('read_data');
                  $becados = $this->read_data->get_pagos_autorizados($this->input->post('id_periodo'));
                  if($becados!=FALSE){
                        $pdf = new TCPDF(PDF_PAGE_ORIENTATION, PDF_UNIT, PDF_PAGE_FORMAT, true, 'UTF-8', false);   
                        ...
                        $pdf->SetCreator(PDF_CREATOR);
                        $pdf->SetAuthor('Patronato');
                        $pdf->SetTitle('Pagos Autorizados.pdf');
                        $pdf->SetSubject('Lista de becados con pago autorizado');
                        $pdf->SetKeywords('Beca, PDF, patronato');  
